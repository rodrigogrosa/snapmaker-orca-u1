...
### MakerWorld por link (macOS)

Conector comunitário independente, referenciado nos projetos OpenBambuAPI e Bambuddy, sem identidade falsa de cliente. Integrações aceita links `https://makerworld.com/[idioma/]models/...`, consulta metadados públicos, autoria/licença, perfis e filamentos por placa. Autenticação global Bambu por e-mail/senha, seguida por código de e-mail ou TOTP/CSRF quando solicitado. A senha fica somente na operação de login; a sessão é salva em `makerworld.token` na mesma pasta privada, separada da credencial Thingiverse. Não há busca por palavras nem suporte à região China nesta versão. CAPTCHA ou recusa de acesso interrompem a operação.

O perfil é validado contra a última consulta. O manifesto autenticado fornece uma URL temporária, consumida imediatamente por uma requisição sem credenciais, sem redirecionamentos e com limite de 200 MiB. Somente hosts MakerWorld/CDN e S3 reconhecidos são aceitos. Nenhum token ou URL assinada é enviado ao JavaScript ou logado pelo conector. O arquivo completo é mantido em `model-library/makerworld` e encaminhado ao importador nativo de projetos. É necessário conferir a compatibilidade do 3MF e selecionar/revisar o perfil U1 no fatiador; esta versão não promete conversão automática de todos os perfis Bambu.

Validação: consulta pública real do modelo MakerWorld 978489 retornou HTTP 200 com perfis, placas e cores. Testes web cobrem login com etapa de código (simulado), exibição de cores, descrições inertes e importação do perfil correto. Teste de arquivo verifica isolamento entre credenciais e sessão de tamanho maior. Login e download autenticados requerem a conta do usuário; não foram validados com dados fictícios como se fossem reais.
